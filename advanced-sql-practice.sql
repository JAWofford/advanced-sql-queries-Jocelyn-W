...
-- checking to see if tables are successfully created
show tables;
select * from customers;
select * from orders;

-- creating an alias with total spent for each customer
SELECT customer_id, SUM(total_amount) AS total_spent
FROM orders
GROUP BY customer_id;

-- how much spent by date
...
-- order greater than $200 using having
SELECT customer_id, SUM(total_amount) AS total_spent
FROM orders
GROUP BY customer_id
HAVING SUM(total_amount) > 200;

-- creating a join on customer-id
...
-- trying out right join even though they didn't tell me to
SELECT *
FROM orders
...
-- table subquery (aka totally redundant subquery)
SELECT order_date
FROM (SELECT id, order_date, total_amount FROM orders) AS order_summary;

-- my test table subquery to find customers who spent more than average
